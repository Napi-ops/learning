...
void GetServer(int Core, int Memory)
{
	while (Core > 0 || Memory > 0)
	{
			double cmratio = (double)Core / (double)Memory;
			Ratio ratio[MAX_SERVER_KIND] = { 0 };
			for (int i = 0; i < server_kind_num; i++)
			{
				ratio[i].value = fabs(server_kind[i].cmratio - cmratio);
				ratio[i].position = i;
			}
			...
			for (int i = 0; i < server_kind_num; i++)
			{
				int pos_core = server_kind[ratio[i].position].core;
				int pos_mem = server_kind[ratio[i].position].memory;
				server[high].server_kind = ratio[i].position;
...
int main()
{
	...
	for (int i = 0; i < days; i++)
	{
		...
		if (tag == false)
		{
			...
			//cout << endl;
			low = high;
			GetServer(totalC, totalM);
		}
		sort(vm + yesterday + 1, vm + today + 1, cmp_sub);
		sort(server + today_low, server + high, cmp_kind);
		int update[MAX_ASK] = { 0 };
		for (int k = 0; k < high; k++)//k=today_low
		{
