...
void vm_in_server(int subscript, int* vm_sub, int& len)
{
	...
	for (int i = 0; i <= yesterday; i++)
		if (vm[i].location == subscript) vm_sub[len++] = i;
}
...
void migration(int& mig_num, Mig_Move* mig_move, int MAX_mig)
{
	...
	for (int i = 0; i < high; i++)
		if (server[i].cmratio == 0)
			good[i] = 0;//perfect don't nees adjustment
		else
			break;
	...
	while (mig_num < MAX_mig)
	{
		...
		{
			int vm_Cm[MAX_ASK / 100], len_Cm = 0;
			vm_in_server(server[i].subscript, vm_Cm, len_Cm);
			sort(vm_Cm, vm_Cm + len_Cm, cmp_Cm);//first one has bigger core and smaller memory

			for (int k = 0; k < len_Cm; k++)
			{
				if (mig_num == MAX_mig) break;
				if (!good[i] && vm_kind[vm[vm_Cm[k]].vm_kind].cmratio > server[i].cmratio)
					for (int l = high - 1; l >= (int)(high * (1 - PERCENT_CHECK)); l--)
						if (!good[l])//need adjustment
						{
							if (adjust(i, l, vm_Cm[k]))
							{
								mig_move[mig_num++] = { vm_Cm[k],server[l].subscript };
		...
		{
			int vm_cM[MAX_ASK / 100], len_cM = 0;
			vm_in_server(server[j].subscript, vm_cM, len_cM);
			sort(vm_cM, vm_cM + len_cM, cmp_cM);//first one has smaller core and bigger memory
			for (int k = 0; k < len_cM; k++)
			{
				if (mig_num == MAX_mig) break;
				if (!good[j] && vm_kind[vm[vm_cM[k]].vm_kind].cmratio < server[j].cmratio)
					for (int l = 0; l <= (int)(high * PERCENT_CHECK); l++)
						if (!good[l])//need adjustment
						{
							if (adjust(j, l, vm_cM[k]))
							{
								mig_move[mig_num++] = { vm_cM[k],server[l].subscript };
...
int main()
{
	...
	for (int i = 0; i < days; i++)
	{
		...
		migration(mig_num, mig_move, vm_sum * 5 / 1000);
		...
		for (int j = 0; j < mig_num; j++)
			if (vm_kind[vm[mig_move[j].vm_subscript].vm_kind].doub == 0)//single
				cout << '(' << vm[mig_move[j].vm_subscript].vm_id << ", " << mig_move[j].server_subcript << ", " << (char)(vm[mig_move[j].vm_subscript].ABtag + 'A') << ")\n";
			else//double
