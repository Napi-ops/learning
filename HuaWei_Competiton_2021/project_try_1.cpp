...
bool greedy_2(int server_id, short& CA, short& MA, short& CB, short& MB, int& Core, int& Memory)
{
	int m = 0;
	int tag = 0;
	for (m = yesterday + 1; m <= today; m++)
	{
		if (vm_kind[vm[m].vm_kind].doub == 0)
			break;
	}
	for (int i = yesterday + 1; i < m; i++)
		if (vm[i].usage == 0)//unlocated double
		{
	...
	double cm = (double)CA / (double)MA;
	Ratio rat[MAX_DAYS] = { 0 };
	for (int i = m; i <= today; i++)
	{
		rat[i - m].value = fabs(vm_kind[vm[i].vm_kind].cmratio - cm);
		rat[i - m].position = i;
	}
	sort(rat, rat + today - m + 1, cmp_ratio);
	for (int i = 0; i <= today - m; i++)
	{
		rat[i].value = i + rat[i].position;
	}
	sort(rat, rat + today - m + 1, cmp_ratio);
		for (int i = 0; i <= today - m; i++)
		{
			if (vm[rat[i].position].usage == 0)
			{
				VM_Kind it = vm_kind[vm[rat[i].position].vm_kind];
				if (it.core <= CA && it.memory <= MA)//can be located in A or B
				{
					tag = 1;
					vm[rat[i].position].usage = 1;
					vm[rat[i].position].location = server_id;
					vm[rat[i].position].ABtag = 0;
					server[server_id].usage = 1;
					CA -= it.core;
					MA -= it.memory;
					Core -= it.core;
...
void GetServer(int Core, int Memory, int day)
{
	...
	while (Core > 0 || Memory > 0)
	{
		...
		for (int i = 0; i < server_kind_num; i++)
		{
			...
			if (greedy_2(high, server[high].core_A, server[high].memory_A, server[high].core_B, server[high].memory_B, Core, Memory))
			{
				++high;
				break;
			}
...
int main()
{
	...
	for (int i = 0; i < vm_kind_num; i++)
	{
		...
		vm_kind[i].cmratio = (double)vm_kind[i].core / (double)vm_kind[i].memory;
	}
